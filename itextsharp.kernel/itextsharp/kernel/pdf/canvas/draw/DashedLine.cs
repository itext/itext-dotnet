...
namespace iTextSharp.Kernel.Pdf.Canvas.Draw
{
	...
	public class DashedLine : ILineDrawer
	{
		...
		{
			canvas.SaveState().SetLineWidth(lineWidth).SetStrokeColor(color).SetLineDash(2, 2
				).MoveTo(drawArea.GetX(), drawArea.GetY() + drawArea.GetHeight() / 2).LineTo(drawArea
				.GetX() + drawArea.GetWidth(), drawArea.GetY() + drawArea.GetHeight() / 2).Stroke
				().RestoreState();
		}
