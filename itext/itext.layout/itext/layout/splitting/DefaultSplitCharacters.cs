...
namespace iText.Layout.Splitting {
    ...
    public class DefaultSplitCharacters : ISplitCharacters {
        ...
        private static readonly ICollection<int> CHARS_NOT_STARTING_LINE = new HashSet<int>(JavaUtil.ArraysAsList(
                // https://www.w3.org/TR/jlreq/?lang=en#cl-02
                0x2019, 0x201D, 0x0029, 0x3015, 0x005D, 0x007D, 0x3009, 0x300B, 0x300D, 0x300F, 0x3011, 0x2986, 0x3019, 0x3017
            , 0x00BB, 0x301F, 
                ...
                0x3041, 0x3043, 0x3045, 0x3047, 0x3049, 0x30A1, 0x30A3, 0x30A5, 0x30A7, 0x30A9, 0x3063, 0x3083, 0x3085, 0x3087
            , 0x308E, 0x3095, 0x3096, 0x30C3, 0x30E3, 0x30E5, 0x30E7, 0x30EE, 0x30F5, 0x30F6, 0x31F0, 0x31F1, 0x31F2
        ...
        private static readonly ICollection<int> CHARS_NOT_ENDING_LINE = new HashSet<int>(JavaUtil.ArraysAsList(
                // https://www.w3.org/TR/jlreq/?lang=en#cl-01
                
                // https://www.w3.org/TR/jlreq/?lang=en#cl-28
                0x2018, 0x201c, 0x0028, 0x3014, 0x005B, 0x007B, 0x3008, 0x300A, 0x300C, 0x300E, 0x3010, 0x2985, 0x3018, 0x3016
            , 0x00AB, 0x301D));

        // https://www.w3.org/TR/jlreq/?lang=en#unbreakable_character_sequences
        private static readonly IDictionary<int, int?> CHARS_INSEPARABLE;

        static DefaultSplitCharacters() {
            CHARS_INSEPARABLE = new Dictionary<int, int?>();
        ...
        public virtual bool IsSplitCharacter(GlyphLine text, int glyphPos) {
            ...
            if (text.Size() - 1 > glyphPos) {
                ...
                int? secondInseparable = CHARS_INSEPARABLE.Get(charCode);
        ...
        private static bool IsDigitChar(GlyphLine text, int glyphPos) {
