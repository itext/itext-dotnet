...
namespace iText.Signatures.Validation {
    ...
    public class OCSPValidator {
        ...
        internal const String ISSUERS_DOES_NOT_MATCH = "OCSP: Issuers doesn't match.";

        internal const String NO_USABLE_OCSP_WAS_FOUND = "No usable OCSP response was found.";
        ...
        /// <summary>Validates a certificate against OCSP Response.</summary>
        ...
        /// <param name="ocspResp">basic OCSP response to check</param>
        /// <param name="verificationDate">verification date to check for</param>
        public virtual void Validate(ValidationReport report, IX509Certificate certificate, IBasicOcspResponse ocspResp
            ...
            bool workingOcspAvailable = false;
            foreach (ISingleResponse iSingleResp in resp) {
                ...
                try {
                    if (!CertificateUtil.CheckIfIssuersMatch(iSingleResp.GetCertID(), (IX509Certificate)issuerCert)) {
                        report.AddReportItem(new CertificateReportItem(certificate, OCSP_CHECK, ISSUERS_DOES_NOT_MATCH, ValidationReport.ValidationResult
                            .VALID));
                        continue;
                    }
                }
                ...
                if (iSingleResp.GetNextUpdate() != null && verificationDate.After(iSingleResp.GetNextUpdate())) {
                    report.AddReportItem(new CertificateReportItem(certificate, OCSP_CHECK, MessageFormatUtil.Format(OCSP_IS_NO_LONGER_VALID
                        , verificationDate, iSingleResp.GetNextUpdate()), ValidationReport.ValidationResult.VALID));
                    continue;
                }
                ...
                if (isStatusGood || (revokedStatus != null && verificationDate.Before(revokedStatus.GetRevocationTime()))) {
                    ...
                    if (!isStatusGood) {
                        report.AddReportItem(new CertificateReportItem(certificate, OCSP_CHECK, MessageFormatUtil.Format(SignLogMessageConstant
                            .VALID_CERTIFICATE_IS_REVOKED, revokedStatus.GetRevocationTime()), ValidationReport.ValidationResult.VALID
                            ));
                    }
                }
                else {
                    if (revokedStatus != null) {
                        report.AddReportItem(new CertificateReportItem(certificate, OCSP_CHECK, CERT_IS_REVOKED, ValidationReport.ValidationResult
                            .INVALID));
                    }
                    else {
                        report.AddReportItem(new CertificateReportItem(certificate, OCSP_CHECK, CERT_STATUS_IS_UNKNOWN, ValidationReport.ValidationResult
                            .INDETERMINATE));
                    }
                }
            }
            if (!workingOcspAvailable) {
                ...
                report.AddReportItem(new CertificateReportItem(certificate, OCSP_CHECK, NO_USABLE_OCSP_WAS_FOUND, ValidationReport.ValidationResult
                    .INDETERMINATE));
            }
        }
        ...
        private void VerifyOcspResponder(ValidationReport report, IBasicOcspResponse ocspResp, IX509Certificate issuerCert
            ...
            if (responderCert == null) {
                ...
                if (responderCert == null) {
                    report.AddReportItem(new CertificateReportItem(issuerCert, OCSP_CHECK, OCSP_COULD_NOT_BE_VERIFIED, ValidationReport.ValidationResult
                        .INDETERMINATE));
                    return;
                }
                if (!certificateRetriever.IsCertificateTrusted(responderCert)) {
                    ...
                    try {
                        ...
                        report.AddReportItem(new CertificateReportItem(responderCert, OCSP_CHECK, INVALID_OCSP, e, ValidationReport.ValidationResult
                            .INVALID));
                        return;
                    }
                }
            ...
            certificateChainValidator.Validate(report, responderCert, ocspResp.GetProducedAt(), JavaCollectionsUtil.SingletonList
                ((CertificateExtension)new ExtendedKeyUsageExtension(JavaCollectionsUtil.SingletonList(ExtendedKeyUsageExtension
                .OCSP_SIGNING))));
        }
    }
}
