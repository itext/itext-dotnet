...
namespace iText.Barcodes {
    public class BarcodeCodabar : Barcode1D {
        ...
        public static byte[] GetBarsCodabar(String text) {
            ...
            int len = text.Length;
            if (len < 2) {
                throw new ArgumentException(PdfException.CODABAR_MUST_HAVE_AT_LEAST_START_AND_STOP_CHARACTER);
            }
            if (CHARS.IndexOf(text[0]) < START_STOP_IDX || CHARS.IndexOf(text[len - 1]) < START_STOP_IDX) {
                throw new ArgumentException(PdfException.CODABAR_MUST_HAVE_ONE_ABCD_AS_START_STOP_CHARACTER);
            }
            byte[] bars = new byte[text.Length * 8 - 1];
            for (int k = 0; k < len; ++k) {
                int idx = CHARS.IndexOf(text[k]);
                if (idx >= START_STOP_IDX && k > 0 && k < len - 1) {
                    throw new ArgumentException(PdfException.IN_CODABAR_START_STOP_CHARACTERS_ARE_ONLY_ALLOWED_AT_THE_EXTREMES
                        );
                }
                if (idx < 0) {
                    throw new ArgumentException(PdfException.ILLEGAL_CHARACTER_IN_CODABAR_BARCODE);
                }
