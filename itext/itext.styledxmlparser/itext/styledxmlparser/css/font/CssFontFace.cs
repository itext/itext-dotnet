...
namespace iText.StyledXmlParser.Css.Font {
    ...
    public class CssFontFace {
        ...
        public class CssFontFaceSrc {
            ...
            public static readonly Regex UrlPattern = iText.Events.Utils.StringUtil.RegexCompile("^((local)|(url))\\(((\'[^\']*\')|(\"[^\"]*\")|([^\'\"\\)]*))\\)( format\\(((\'[^\']*\')|(\"[^\"]*\")|([^\'\"\\)]*))\\))?$"
                );
            ...
            */
            ...
            public static CssFontFace.CssFontFaceSrc Create(String src) {
                Matcher m = iText.Events.Utils.Matcher.Match(UrlPattern, src);
                if (!m.Matches()) {
                    return null;
                }
