...
namespace iText.StyledXmlParser.Jsoup.Select {
    ...
    public class QueryParser {
                        ...
                        if (cq.MatchChomp("^=")) {
                            ...
                            if (cq.MatchChomp("$=")) {
                                ...
                                if (cq.MatchChomp("*=")) {
                                    ...
                                    if (cq.MatchChomp("~=")) {
                                        evals.Add(new Evaluator.AttributeWithValueMatching(key, iText.Events.Utils.StringUtil.RegexCompile(cq.Remainder
                                            ())));
                                    }
                                    else {
        ...
        private static readonly Regex NTH_AB = iText.Events.Utils.StringUtil.RegexCompile("(([+-])?(\\d+)?)n(\\s*([+-])?\\s*\\d+)?"
            , System.Text.RegularExpressions.RegexOptions.IgnoreCase);

        private static readonly Regex NTH_B = iText.Events.Utils.StringUtil.RegexCompile("([+-])?(\\d+)");

        private void CssNthChild(bool backwards, bool ofType) {
            String argS = Normalizer.Normalize(tq.ChompTo(")"));
            Matcher mAB = iText.Events.Utils.Matcher.Match(NTH_AB, argS);
            Matcher mB = iText.Events.Utils.Matcher.Match(NTH_B, argS);
            int a;
            int b;
            if ("odd".Equals(argS)) {
                a = 2;
        ...
        private void Matches(bool own) {
            ...
            if (own) {
                evals.Add(new Evaluator.MatchesOwn(iText.Events.Utils.StringUtil.RegexCompile(regex)));
            }
            else {
                evals.Add(new MatchesElement(iText.Events.Utils.StringUtil.RegexCompile(regex)));
            }
        }
