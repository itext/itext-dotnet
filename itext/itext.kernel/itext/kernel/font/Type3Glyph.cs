...
namespace iText.Kernel.Font {
    ...
    public sealed class Type3Glyph : PdfCanvas {
        ...
        private void FillBBFromBytes(byte[] bytes) {
            String str = iText.Events.Utils.JavaUtil.GetStringForBytes(bytes, iText.Events.Utils.EncodingUtil.ISO_8859_1
                );
            int d0Pos = str.IndexOf(D_0_STR, StringComparison.Ordinal);
            int d1Pos = str.IndexOf(D_1_STR, StringComparison.Ordinal);
            if (d0Pos != -1) {
                isColor = true;
                String[] bbArray = iText.Events.Utils.StringUtil.Split(str.JSubstring(0, d0Pos - 1), " ");
                if (bbArray.Length == 2) {
                ...
                if (d1Pos != -1) {
                    isColor = false;
                    String[] bbArray = iText.Events.Utils.StringUtil.Split(str.JSubstring(0, d1Pos - 1), " ");
                    if (bbArray.Length == 6) {
                        this.wx = float.Parse(bbArray[0], System.Globalization.CultureInfo.InvariantCulture);
