...
namespace iText.Kernel.Utils {
    ...
    public class PageRange {
        private static readonly Regex SEQUENCE_PATTERN = iText.Events.Utils.StringUtil.RegexCompile("(\\d+)-(\\d+)?"
            );

        private static readonly Regex SINGLE_PAGE_PATTERN = iText.Events.Utils.StringUtil.RegexCompile("(\\d+)");
        ...
        public PageRange(String pageRange) {
            pageRange = iText.Events.Utils.StringUtil.ReplaceAll(pageRange, "\\s+", "");
            foreach (String pageRangePart in iText.Events.Utils.StringUtil.Split(pageRange, ",")) {
        ...
        private static PageRange.IPageRangePart GetRangeObject(String rangeDef) {
            if (rangeDef.Contains("&")) {
                ...
                foreach (String pageRangeCond in iText.Events.Utils.StringUtil.Split(rangeDef, "&")) {
                ...
                Matcher matcher;
                if ((matcher = iText.Events.Utils.Matcher.Match(SEQUENCE_PATTERN, rangeDef)).Matches()) {
                    ...
                    if ((matcher = iText.Events.Utils.Matcher.Match(SINGLE_PAGE_PATTERN, rangeDef)).Matches()) {
