...
namespace iText.Kernel.Colors.Gradients {
    ...
    public abstract class AbstractGradientBuilder<T> {
        ...
        private const double ZERO_EPSILON = 1E-10;
        ...
        /// <see cref="AbstractGradientBuilder{T}">more info</see>
        /// ).
        /// </summary>
        /// <remarks>
        /// Adds the new color stop to the end (
        ...
        public virtual AbstractGradientBuilder<T> AddStopColor(GradientColorStop gradientColorStop) {
        ...
        /// <returns>the current builder instance</returns>
        public virtual AbstractGradientBuilder<T> SetSpread(GradientSpreadMethod gradientSpreadMethod) {
        ...
        public virtual IList<GradientColorStop> GetStopColors() {
        ...
        public virtual GradientSpreadMethod GetSpread() {
        ...
        public virtual Color BuildColor(Rectangle targetBoundingBox, AffineTransform contextTransform, PdfDocument
            ...
            Tuple2<T[], AffineTransform> gradientVectorWithTransform = GetGradientVectorWithTransform(targetBoundingBox
                , contextTransform);
            ...
            if (gradientTransformation != null) {
                try {
                    ...
                    new LazyLogger(GetType()).Error(() => iText.IO.Logs.IoLogMessageConstant.UNABLE_TO_INVERT_GRADIENT_TRANSFORMATION
                        );
                }
            }
        ...
        /// <returns>
        /// the pair of the vector, which is an array of exactly two elements specifying
        /// the gradient coordinates vector, and the additional transformation to be concatenated to
        /// the current for resulted shading or
        /// <see langword="null"/>
        /// if no additional transformation is specified
        /// </returns>
        protected internal virtual Tuple2<T[], AffineTransform> GetGradientVectorWithTransform(Rectangle targetBoundingBox
        ...
        /// <summary>Computes the minimal domain that covers the box with vector normals.</summary>
        /// <remarks>
        ...
        /// domain
        /// </returns>
        protected internal abstract double[] ComputeCoveringDomain(T[] coords, Rectangle toCover);
        ...
        protected internal abstract T[] CreateCoordsForNewDomain(double[] newDomain, T[] baseVector);

        /// <summary>Creates some gradient vector which would cover the target bounding box.</summary>
        /// <param name="targetBoundingBox">the bounding box to cover</param>
        /// <returns>constructed gradient vector</returns>
        protected internal abstract T[] CreateCoveringCoordinates(Rectangle targetBoundingBox);

        /// <summary>Evaluates base vector length for future color stops spreading.</summary>
        /// <param name="coordinates">the base vector</param>
        /// <returns>base vector length</returns>
        protected internal abstract double GetBaseVectorLength(T[] coordinates);

        /// <summary>Converts gradient vector into pdf coordinates array for shading.</summary>
        /// <param name="coordinates">vector to convert</param>
        /// <returns>constructed pdf array</returns>
        protected internal abstract PdfArray CreateCoordsDictEntry(T[] coordinates);

        /// <summary>Creates the gradient shading object.</summary>
        /// <param name="colorSpace">colorspace to use</param>
        /// <param name="coordinates">gradient coordinates</param>
        /// <param name="coordinatesDomain">gradient coordinates domain</param>
        /// <param name="stopsFunction">color stops function</param>
        /// <returns>constructed gradient shading</returns>
        protected internal abstract AbstractPdfShading CreatePdfShading(PdfColorSpace colorSpace, PdfArray coordinates
            , PdfArray coordinatesDomain, IPdfFunction stopsFunction);

        /// <summary>
        /// Constructs
        /// <see cref="iText.Kernel.Geom.AffineTransform"/>
        /// which represents transformation from current coordinates
        /// into the coordinates in which domainStart would be (0,0), domain end would be on X axes
        /// (i.e. (X,0) coordinate) and the surface scaled both on X and Y by
        /// <paramref name="scale"/>
        /// factor.
        /// </summary>
        /// <param name="domainStart">the point to become (0,0)</param>
        /// <param name="domainEnd">the point to be placed on X axes</param>
        /// <param name="scale">the scale factor</param>
        /// <returns>the constructed transformation</returns>
        protected internal static AffineTransform GetToIntervalTransform(Point domainStart, Point domainEnd, double
             scale) {
        ...
        /// <summary>Checks whether the argument is close to 0</summary>
        /// <param name="d">to check</param>
        /// <returns>
        /// 
        /// <see langword="true"/>
        /// if the argument is close to 0
        /// </returns>
        protected internal static bool IsZero(double d) {
            return Math.Abs(d) < ZERO_EPSILON;
        }

        private AbstractPdfShading ConstructPdfShading(T[] baseCoordinatesVector, IList<GradientColorStop> stops, 
            GradientSpreadMethod spreadMethod, Rectangle targetBoundingBox, PdfDocument pdfDocument) {
            double baseVectorLength = GetBaseVectorLength(baseCoordinatesVector);
            IList<GradientColorStop> normalizedStops = NormalizeStops(stops, baseVectorLength);
            IList<GradientColorStop> stopsToConstruct;
            double[] coordinatesDomain = new double[] { 0, 1 };
            T[] actualCoordinates;
            if (IsZero(baseVectorLength) || normalizedStops.Count == 1) {
                // single color case
                ...
                if (spreadMethod == GradientSpreadMethod.REPEAT || spreadMethod == GradientSpreadMethod.REFLECT) {
                    stopsToConstruct = AdjustNormalizedStopsToCoverDomain(normalizedStops, coordinatesDomain, spreadMethod, pdfDocument
                        );
                }
                else {
                    if (spreadMethod == GradientSpreadMethod.PAD) {
                        ...
                        if (IsZero(lastStopOffset - firstStopOffset) || coordinatesDomain[1] <= firstStopOffset || coordinatesDomain
                            [0] >= lastStopOffset) {
                            return null;
                        }
        ...
        private static IList<GradientColorStop> AdjustStopsForPadIfNeeded(IList<GradientColorStop> normalizedStops
            , double[] coordinatesDomain) {
            IList<GradientColorStop> stopsToConstruct = new List<GradientColorStop>(normalizedStops.Count + 2);
            GradientColorStop firstStop = normalizedStops[0];
            ...
            GradientColorStop lastStop = normalizedStops[normalizedStops.Count - 1];
        ...
        private IList<GradientColorStop> AdjustNormalizedStopsToCoverDomain(IList<GradientColorStop> normalizedStops
            ...
            while (lastComputedOffset <= targetDomain[1]) {
                ...
                if (adjustedStops.Count > maxStopCount) {
                    new LazyLogger(GetType()).Warn(() => MessageFormatUtil.Format(KernelLogMessageConstant.GRADIENT_MAX_COLOR_STOPS
                        , maxStopCount));
        ...
        private static int GetMaxStopCountForRepeatAndReflect(PdfDocument pdfDocument) {
            ...
            if (pdfDocument != null) {
                GradientPropertiesResolver gradientPropertiesResolver = pdfDocument.GetDiContainer().GetInstance<GradientPropertiesResolver
                    >();
