...
namespace iText.Events.Producer {
    ...
    internal class CurrentDatePlaceholderPopulator : AbstractFormattedPlaceholderPopulator {
        ...
        public override String Populate(IList<ConfirmedEventWrapper> events, String parameter) {
            if (parameter == null) {
                throw new ArgumentException(MessageFormatUtil.Format(EventsExceptionMessageConstant.INVALID_USAGE_FORMAT_REQUIRED
                    , "currentDate"));
            }
        ...
        private int ProcessDateComponent(int index, DateTime date, StringBuilder builder, char[] formatArray) {
            ...
            if (ALLOWED_PATTERNS.Contains(piece)) {
                ...
                throw new ArgumentException(MessageFormatUtil.Format(EventsExceptionMessageConstant.PATTERN_CONTAINS_UNEXPECTED_COMPONENT
                    , piece));
            }
            return index;
        }
