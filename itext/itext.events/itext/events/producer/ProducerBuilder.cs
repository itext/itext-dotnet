...
namespace iText.Events.Producer {
    ...
    public sealed class ProducerBuilder {
        private static readonly ILog LOGGER = LogManager.GetLogger(typeof(iText.Events.Producer.ProducerBuilder));
        ...
        /// <see cref="iText.Events.Confirmations.ConfirmedEventWrapper"/>
        /// registered for
        /// the document
        /// </param>
        ...
        private static String BuildProducer(IList<ConfirmedEventWrapper> events) {
            if (events == null || events.IsEmpty()) {
                throw new ArgumentException(EventsExceptionMessageConstant.NO_EVENTS_WERE_REGISTERED_FOR_THE_DOCUMENT);
            }
        ...
        private static String PopulatePlaceholders(String producerLine, IList<ConfirmedEventWrapper> events) {
            int lastIndex = 0;
            Matcher matcher = iText.Events.Utils.Matcher.Match(PATTERN, producerLine);
            ...
            while (matcher.Find()) {
                ...
                if (populator == null) {
                    LOGGER.Info(MessageFormatUtil.Format(EventsLogMessageConstant.UNKNOWN_PLACEHOLDER_WAS_IGNORED, placeholderName
                        ));
                }
                else {
