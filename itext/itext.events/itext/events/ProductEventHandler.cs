...
namespace iText.Events {
    ...
    internal sealed class ProductEventHandler : AbstractContextBasedEventHandler {
        internal static readonly iText.Events.ProductEventHandler INSTANCE = new iText.Events.ProductEventHandler(
            );

        private static readonly ILog LOGGER = LogManager.GetLogger(typeof(iText.Events.ProductEventHandler));
        ...
        /// <see cref="iText.Events.Processors.ITextProductEventProcessor"/>.
        /// </summary>
        ...
        private void WrapConfirmedEvent(ConfirmEvent @event, ITextProductEventProcessor productEventProcessor) {
            lock (events) {
                ...
                if (indexOfReportedEvent >= 0) {
                    ...
                    LOGGER.Warn(MessageFormatUtil.Format(EventsLogMessageConstant.UNREPORTED_EVENT, confirmedEvent.GetProductName
