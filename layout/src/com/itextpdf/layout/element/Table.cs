...
namespace com.itextpdf.layout.element
{
	...
	/// </summary>
	public class Table : BlockElement<com.itextpdf.layout.element.Table>, ILargeElement
	{
		...
		private com.itextpdf.layout.element.Table header;

		private com.itextpdf.layout.element.Table footer;
		...
		/// <param name="largeTable">whether parts of the table will be written before all data is added.
		/// 	</param>
		...
		/// hasn't been called.
		/// </returns>
		public virtual com.itextpdf.layout.element.Table GetHeader()
		{
			return header;
		}
		...
		/// hasn't been called.
		/// </returns>
		public virtual com.itextpdf.layout.element.Table GetFooter()
		{
			return footer;
		}
		...
		/// <returns>this element</returns>
		public virtual com.itextpdf.layout.element.Table SetSkipFirstHeader(bool skipFirstHeader
			)
		{
		...
		/// <returns>this element</returns>
		public virtual com.itextpdf.layout.element.Table SetSkipLastFooter(bool skipLastFooter
			)
		{
		...
		/// 	</remarks>
		/// <returns>this element</returns>
		public virtual com.itextpdf.layout.element.Table StartNewRow()
		{
			currentColumn = 0;
			currentRow++;
		...
		/// <returns>this element</returns>
		public virtual com.itextpdf.layout.element.Table AddCell(Image image)
		{
		...
		/// <returns>this element</returns>
		public virtual com.itextpdf.layout.element.Table AddCell(String content)
		{
		...
		/// <returns>
		/// a
		/// <see cref="com.itextpdf.layout.renderer.TableRenderer"/>
		...
		public override IRenderer GetRenderer()
		{
			if (nextRenderer != null)
			{
				...
				{
					Logger logger = LoggerFactory.GetLogger(typeof(com.itextpdf.layout.element.Table)
						);
			...
			if (isComplete)
			{
				return new TableRenderer(this, new Table.RowRange(rowWindowStart, rowWindowStart 
					+ rows.Count - 1));
			}
			else
			{
				int rowWindowFinish = lastAddedRowGroups.Count != 0 ? lastAddedRowGroups[lastAddedRowGroups
					.Count - 1].finishRow : -1;
		...
		public virtual List<Border> GetLastRowBottomBorder()
		{
			...
			if (lastAddedRow != null)
			{
				for (int i = 0; i < lastAddedRow.Length; i++)
				{
					...
					if (cell != null)
					{
						Border border = ((Border)cell.GetProperty(Property.BORDER));
						if (border == null)
						{
							border = ((Border)cell.GetProperty(Property.BORDER_BOTTOM));
						}
		...
		{
			int lastRowWeCanFlush = currentColumn == columnWidths.Length ? currentRow : currentRow
				 - 1;
			...
			while (currentRowGroupStart <= lastRowWeCanFlush)
			{
				...
				int maxRowGroupFinish = cellBottomRows[0] + rows[cellBottomRows[0] - rowWindowStart
					][0].GetRowspan() - 1;
				...
				while (!converged)
				{
					...
					{
						while (cellBottomRows[i_1] < lastRowWeCanFlush && cellBottomRows[i_1] + rows[cellBottomRows
							[i_1] - rowWindowStart][i_1].GetRowspan() - 1 < maxRowGroupFinish)
						{
							cellBottomRows[i_1] += rows[cellBottomRows[i_1] - rowWindowStart][i_1].GetRowspan
								();
						}
						if (cellBottomRows[i_1] + rows[cellBottomRows[i_1] - rowWindowStart][i_1].GetRowspan
							() - 1 > maxRowGroupFinish)
						{
							maxRowGroupFinish = cellBottomRows[i_1] + rows[cellBottomRows[i_1] - rowWindowStart
								][i_1].GetRowspan() - 1;
							converged = false;
						}
						else
		...
		private void EnsureHeaderIsInitialized()
		{
			if (header == null)
			{
				header = new com.itextpdf.layout.element.Table(columnWidths);
		...
		private void EnsureFooterIsInitialized()
		{
			if (footer == null)
			{
				footer = new com.itextpdf.layout.element.Table(columnWidths);
