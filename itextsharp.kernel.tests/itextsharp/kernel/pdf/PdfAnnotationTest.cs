...
namespace iTextSharp.Kernel.Pdf
{
	...
	{
		public const String sourceFolder = "../../resources/itextsharp/kernel/pdf/PdfAnnotationTest/";

		public const String destinationFolder = "test/itextsharp/kernel/pdf/PdfAnnotationTest/";
		...
		public virtual void AddLinkAnnotation01()
		{
			...
			page1.AddAnnotation(new PdfLinkAnnotation(new Rectangle(100, 560, 260, 25)).SetDestination
				(PdfExplicitDestination.CreateFit(page2)).SetBorder(new PdfArray(new float[] { 0
				, 0, 1 })));
			page1.Flush();
		...
		public virtual void AddLinkAnnotation02()
		{
			...
			page.AddAnnotation(((PdfLinkAnnotation)new PdfLinkAnnotation(new Rectangle(100, 590
				, 300, 25)).SetAction(PdfAction.CreateURI("http://itextpdf.com"))).SetBorder(new 
				PdfArray(new float[] { 0, 0, 1 })).SetColor(new PdfArray(new float[] { 1, 0, 0 }
				)));
			page.Flush();
			document.Close();
			NUnit.Framework.Assert.IsNull(new CompareTool().CompareByContent(destinationFolder
		...
		public virtual void AddAndGetLinkAnnotations()
		{
			...
			page.AddAnnotation(((PdfLinkAnnotation)new PdfLinkAnnotation(new Rectangle(100, 590
				, 300, 25)).SetAction(PdfAction.CreateURI("http://itextpdf.com"))).SetBorder(new 
				PdfArray(borders)).SetColor(new PdfArray(new float[] { 1, 0, 0 })));
		...
		public virtual void AddFreeTextAnnotation01()
		{
			...
			document.Close();
			NUnit.Framework.Assert.IsNull(new CompareTool().CompareByContent(destinationFolder
				 + "freeTextAnnotation01.pdf", sourceFolder + "cmp_freeTextAnnotation01.pdf", destinationFolder
				, "diff_"));
		}
		...
		public virtual void RedactionTest()
		{
			...
			canvasN.SetColor(iTextSharp.Kernel.Color.Color.RED, true).SetLineWidth(1.5f).SetLineCapStyle
				(PdfCanvasConstants.LineCapStyle.PROJECTING_SQUARE).Rectangle(180, 531, 120, 48)
				.Stroke().Rectangle(181, 532, 118, 47).ClosePath();
