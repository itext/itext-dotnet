	...
	public class PdfLayerTest : ExtendedITextTest
	{
		...
		public virtual void TestInStamperMode1()
		{
			PdfDocument pdfDoc = new PdfDocument(new PdfReader(new FileStream(sourceFolder + 
				"input_layered.pdf")), new PdfWriter(new FileOutputStream(destinationFolder + "output_copy_layered.pdf"
				)));
			pdfDoc.Close();
		...
		{
			PdfDocument pdfDoc = new PdfDocument(new PdfReader(new FileStream(sourceFolder + 
				"input_layered.pdf")), new PdfWriter(new FileOutputStream(destinationFolder + "output_layered.pdf"
				)));
