...
namespace iTextSharp.Kernel.Pdf
{
	public class TagTreePointerTest : ExtendedITextTest
	{
		...
		public virtual void TagTreePointerTest01()
		{
			...
			canvas.BeginText().SetFontAndSize(standardFont, 24).SetTextMatrix(1, 0, 0, 1, 32, 
				512);
			tagPointer.AddTag(PdfName.P).AddTag(PdfName.Span);
			canvas.OpenTag(tagPointer.GetTagReference()).ShowText("Hello ").CloseTag();
			canvas.SetFontAndSize(standardFont, 30).OpenTag(tagPointer.GetTagReference()).ShowText
				("World").CloseTag();
			...
			canvas.BeginText().SetFontAndSize(PdfFontFactory.CreateFont(FontConstants.HELVETICA
				), 24).SetTextMatrix(1, 0, 0, 1, 32, 512);
			tagPointer.AddTag(PdfName.P).AddTag(PdfName.Span);
			canvas.OpenTag(tagPointer.GetTagReference()).ShowText("Hello ").CloseTag();
		...
		{
			PdfReader reader = new PdfReader(sourceFolder + "taggedDocument.pdf");
			PdfWriter writer = new PdfWriter(destinationFolder + "tagTreePointerTest04.pdf").
				SetCompressionLevel(CompressionConstants.NO_COMPRESSION);
			PdfDocument document = new PdfDocument(reader, writer);
			TagTreePointer tagPointer = new TagTreePointer(document);
			tagPointer.MoveToKid(PdfName.Table).MoveToKid(2, PdfName.TR);
			tagPointer.RemoveTag();
			tagPointer.MoveToKid(PdfName.TR).MoveToKid(PdfName.TD).MoveToKid(PdfName.P).MoveToKid
				(PdfName.Span);
		...
		public virtual void TagTreePointerTest05()
		{
			...
			tagPointer1.MoveToParent().MoveToKid(5, PdfName.TR).MoveToKid(2, PdfName.TD).MoveToKid
				(PdfName.P).MoveToKid(PdfName.Span);
		...
		public virtual void TagTreePointerTest06()
		{
			...
			tagPointer.MoveToRoot().MoveToKid(PdfName.Table).MoveToKid(1, PdfName.TR).GetProperties
				().SetActualText("More latin text");
			connectedElement.SetRole(PdfName.Div);
			connectedElement.GetAccessibilityProperties().SetLanguage("en-Us");
			NUnit.Framework.Assert.AreEqual(connectedElement.GetAccessibilityProperties().GetActualText
				(), actualText1);
			document.Close();
		...
		public virtual void TagStructureFlushingTest01()
		{
			...
			document.Close();
			NUnit.Framework.Assert.AreEqual(PdfException.CannotFlushDocumentRootTagBeforeDocumentIsClosed
				, exceptionMessage);
		...
		public virtual void TagStructureFlushingTest02()
		{
			...
			NUnit.Framework.Assert.IsTrue(!((PdfStructElem)kids[0].GetKids()[0]).GetPdfObject
				().IsFlushed());
		...
		public virtual void TagStructureFlushingTest03()
		{
			...
			NUnit.Framework.Assert.IsTrue(kids.GetAsDictionary(0).GetAsDictionary(PdfName.K).
				IsFlushed());
			document.Close();
		...
		public virtual void TagStructureFlushingTest05()
		{
			...
			page2.Flush();
			document.Close();
			CompareResult("tagStructureFlushingTest05.pdf", "cmp_tagStructureFlushingTest05.pdf"
				, "diffFlushing05_");
		}
		...
		public virtual void TagStructureRemovingTest01()
		{
			...
			document.Close();
			CompareResult("tagStructureRemovingTest01.pdf", "cmp_tagStructureRemovingTest01.pdf"
				, "diffRemoving01_");
		}
		...
		public virtual void TagStructureRemovingTest02()
		{
			...
			canvas.OpenTag(tagPointer.GetTagReference()).ShowText("Hello ").CloseTag();
			canvas.SetFontAndSize(standardFont, 30).OpenTag(tagPointer.GetTagReference()).ShowText
				("World").CloseTag().EndText();
			document.Close();
			CompareResult("tagStructureRemovingTest02.pdf", "cmp_tagStructureRemovingTest02.pdf"
				, "diffRemoving02_");
		}
		...
		public virtual void TagStructureRemovingTest03()
		{
			...
			canvas.OpenTag(tagPointer.GetTagReference()).BeginText().SetFontAndSize(standardFont
				, 24).SetTextMatrix(1, 0, 0, 1, 32, 512).ShowText("Hello.").EndText().CloseTag();
			document.Close();
			CompareResult("tagStructureRemovingTest03.pdf", "cmp_tagStructureRemovingTest03.pdf"
				, "diffRemoving03_");
		}
		...
		public virtual void TagStructureRemovingTest04()
		{
			...
			PdfWriter writer = new PdfWriter(destinationFolder + "tagStructureRemovingTest04.pdf"
				).SetCompressionLevel(CompressionConstants.NO_COMPRESSION);
			PdfDocument document = new PdfDocument(reader, writer);
			document.RemovePage(1);
			document.Close();
			CompareResult("tagStructureRemovingTest04.pdf", "cmp_tagStructureRemovingTest04.pdf"
				, "diffRemoving04_");
		}
