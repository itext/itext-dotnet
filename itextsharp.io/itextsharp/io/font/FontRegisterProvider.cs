...
namespace iTextSharp.IO.Font
{
    ...
    internal class FontRegisterProvider
    {
        ...
        public virtual void RegisterFontFamily(String familyName, String fullName, String
             path)
        {
            if (path != null)
            {
        ...
        public virtual void RegisterFont(String path, String alias)
        {
            try
            {
                if (path.ToLower(System.Globalization.CultureInfo.InvariantCulture).EndsWith(".ttf"
                    ) || path.ToLower(System.Globalization.CultureInfo.InvariantCulture).EndsWith
                    (".otf") || path.ToLower(System.Globalization.CultureInfo.InvariantCulture).IndexOf
                    (".ttc,") > 0)
                {
