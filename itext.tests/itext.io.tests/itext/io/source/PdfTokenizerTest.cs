...
namespace iText.IO.Source {
    public class PdfTokenizerTest : ExtendedITextTest {
        ...
        public virtual void SeekTest() {
            ...
            PdfTokenizer tok = new PdfTokenizer(new RandomAccessFileOrArray(factory.CreateSource(data.GetBytes(iText.Events.Utils.EncodingUtil.ISO_8859_1
                ))));
            tok.Seek(0);
            tok.NextValidToken();
        ...
        public virtual void ReadTest() {
            ...
            NUnit.Framework.Assert.AreEqual("/Name1 ", iText.Events.Utils.JavaUtil.GetStringForBytes(read));
        }
        ...
        public virtual void GetDecodedStringContentTest() {
            ...
            tok.NextToken();
            NUnit.Framework.Assert.AreEqual("Name1", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetDecodedStringContent
                ()));
            tok.NextToken();
            NUnit.Framework.Assert.AreEqual("15", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetDecodedStringContent
                ()));
            tok.NextToken();
            NUnit.Framework.Assert.AreEqual("", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetDecodedStringContent
                ()));
        }
        ...
        public virtual void GetDecodedStringContentHexTest() {
            ...
            NUnit.Framework.Assert.AreEqual("some hex string", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetDecodedStringContent
                ()));
        }
        ...
        public virtual void ThrowErrorTest() {
            RandomAccessSourceFactory factory = new RandomAccessSourceFactory();
            PdfTokenizer tok = new PdfTokenizer(new RandomAccessFileOrArray(factory.CreateSource("/Name1".GetBytes(iText.Events.Utils.EncodingUtil.ISO_8859_1
                ))));
        ...
        public virtual void TokenValueEqualsToTest() {
            ...
            tok.NextToken();
            NUnit.Framework.Assert.IsTrue(tok.TokenValueEqualsTo(data.GetBytes(iText.Events.Utils.EncodingUtil.ISO_8859_1
                )));
        }
        ...
        public virtual void TokenValueEqualsToNotSameStringTest() {
            ...
            tok.NextToken();
            NUnit.Framework.Assert.IsFalse(tok.TokenValueEqualsTo((data + "s").GetBytes(iText.Events.Utils.EncodingUtil.ISO_8859_1
                )));
        }
        ...
        public virtual void TokenValueEqualsToNotCaseSensitiveStringTest() {
            ...
            tok.NextToken();
            NUnit.Framework.Assert.IsFalse(tok.TokenValueEqualsTo("Somestring".GetBytes(iText.Events.Utils.EncodingUtil.ISO_8859_1
                )));
        }
        ...
        public virtual void PrimitivesTest() {
            ...
            NUnit.Framework.Assert.AreEqual("Size", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent()
                ));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Number, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("70.", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent())
                );
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Name, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("Value#20", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent
                ()));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Number, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual(".1", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent()));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Name, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("Root", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent()
                ));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Ref, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("46 0 R", "" + tok.GetObjNr() + " " + tok.GetGenNr() + " " + iText.Events.Utils.JavaUtil.GetStringForBytes
                (tok.GetByteContent()));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Name, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("Info", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent()
                ));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Ref, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("44 0 R", "" + tok.GetObjNr() + " " + tok.GetGenNr() + " " + iText.Events.Utils.JavaUtil.GetStringForBytes
                (tok.GetByteContent()));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Name, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("ID", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent()));
            tok.NextValidToken();
            ...
            NUnit.Framework.Assert.IsTrue(tok.IsHexString());
            NUnit.Framework.Assert.AreEqual("736f6d652068657820737472696e672", iText.Events.Utils.JavaUtil.GetStringForBytes
                (tok.GetByteContent()));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.String, tok.GetTokenType());
            NUnit.Framework.Assert.IsFalse(tok.IsHexString());
            NUnit.Framework.Assert.AreEqual("some simple string ", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent
                ()));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.String, tok.GetTokenType());
            NUnit.Framework.Assert.IsTrue(tok.IsHexString());
            NUnit.Framework.Assert.AreEqual("8C2547D58D4BD2C6F3D32B830BE3259D2", iText.Events.Utils.JavaUtil.GetStringForBytes
                (tok.GetByteContent()));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Number, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("-70.1", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent(
                )));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Number, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("-0.2", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent()
                ));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.EndArray, tok.GetTokenType());
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Name, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("Name1", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent(
                )));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Number, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("0", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent()));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Name, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("Prev", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent()
                ));
            tok.NextValidToken();
            NUnit.Framework.Assert.AreEqual(PdfTokenizer.TokenType.Number, tok.GetTokenType());
            NUnit.Framework.Assert.AreEqual("-116.23", iText.Events.Utils.JavaUtil.GetStringForBytes(tok.GetByteContent
                ()));
        }
