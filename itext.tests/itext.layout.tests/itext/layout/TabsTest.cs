...
namespace iText.Layout {
    public class TabsTest : ExtendedITextTest {
        ...
        private void AddTabbedTextToParagraph(Paragraph p, String text, float[] positions, TabAlignment[] alignments
            ...
            p.AddTabStops(tabStops);
            foreach (String line in iText.Events.Utils.StringUtil.Split(text, "\n")) {
                foreach (String chunk in iText.Events.Utils.StringUtil.Split(line, "\t")) {
                    foreach (String piece in iText.Events.Utils.StringUtil.Split(chunk, "#")) {
