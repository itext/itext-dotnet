...
namespace iTextSharp.Kernel.Pdf.Canvas.Parser
{
    public class FilteredEventListenerTest : ExtendedITextTest
    {
        ...
        {
            PdfDocument pdfDocument = new PdfDocument(new PdfReader(new FileStream(sourceFolder + "test.pdf", FileMode.Open
                , FileAccess.Read)));
            ...
            {
                extractionStrategies[i_1] = listener.AttachEventListener(new LocationTextExtractionStrategy(), regionFilters
                    [i_1]);
            }
