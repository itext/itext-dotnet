...
namespace iText.Kernel.Pdf {
    public class XMPMetadataTest : ExtendedITextTest {
        ...
        public virtual void CreateEmptyDocumentWithAbcXmp() {
            ...
            PdfPage page = pdfDoc.AddNewPage();
            page.Flush();
            pdfDoc.SetXmpMetadata("abc".GetBytes(iText.Events.Util.EncodingUtil.ISO_8859_1));
            pdfDoc.Close();
            PdfReader reader = new PdfReader(new MemoryStream(fos.ToArray()));
            PdfDocument pdfDocument = new PdfDocument(reader);
            NUnit.Framework.Assert.AreEqual(false, reader.HasRebuiltXref(), "Rebuilt");
            NUnit.Framework.Assert.AreEqual("abc".GetBytes(iText.Events.Util.EncodingUtil.ISO_8859_1), pdfDocument.GetXmpMetadata
                ());
        ...
        private void RunCustomXmpTest(String name, String xmp) {
            ...
            PdfPage page = pdfDoc.AddNewPage();
            page.Flush();
            pdfDoc.SetXmpMetadata(xmp.GetBytes(iText.Events.Util.EncodingUtil.ISO_8859_1));
            pdfDoc.Close();
