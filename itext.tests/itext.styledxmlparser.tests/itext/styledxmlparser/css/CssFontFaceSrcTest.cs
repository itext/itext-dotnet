...
namespace iText.StyledXmlParser.Css {
    public class CssFontFaceSrcTest : ExtendedITextTest {
        ...
        public virtual void SrcPropertyTest() {
            ...
            String[] sources = iText.Events.Utils.StringUtil.Split(src.GetExpression(), ",");
            NUnit.Framework.Assert.AreEqual(27, sources.Length, "27 sources expected");
            for (int i = 0; i < sources.Length; i++) {
                Matcher m = iText.Events.Utils.Matcher.Match(CssFontFace.CssFontFaceSrc.UrlPattern, sources[i]);
