...
namespace iText.Signatures.Validation {
    ...
    public class RevocationDataValidatorTest : ExtendedITextTest {
        ...
        public virtual void BasicValidationWithOcspClientTest() {
            ...
            RevocationDataValidator validator = new RevocationDataValidator().SetIssuingCertificateRetriever(certificateRetriever
                ).SetOnlineFetching(RevocationDataValidator.OnlineFetching.NEVER_FETCH).AddOcspClient(ocspClient);
        ...
        public virtual void BasicValidationWithCrlClientTest() {
            ...
            certificateRetriever.AddTrustedCertificates(JavaCollectionsUtil.SingletonList(caCert));
            RevocationDataValidator validator = new RevocationDataValidator().SetIssuingCertificateRetriever(certificateRetriever
                ).SetOnlineFetching(RevocationDataValidator.OnlineFetching.NEVER_FETCH).AddCrlClient(crlClient);
        ...
        public virtual void UseFreshCrlResponseTest() {
            ...
            certificateRetriever.AddTrustedCertificates(JavaCollectionsUtil.SingletonList(caCert));
            RevocationDataValidator validator = new RevocationDataValidator().SetIssuingCertificateRetriever(certificateRetriever
                ).SetOnlineFetching(RevocationDataValidator.OnlineFetching.NEVER_FETCH).AddCrlClient(crlClient1).AddCrlClient
                (crlClient2);
            validator.SetFreshness(0);
            validator.Validate(report, checkCert, checkDate);
            NUnit.Framework.Assert.AreEqual(1, report.GetFailures().Count);
        ...
        public virtual void UseFreshOcspResponseTest() {
            ...
            certificateRetriever.AddTrustedCertificates(JavaCollectionsUtil.SingletonList(caCert));
            RevocationDataValidator validator = new RevocationDataValidator().SetIssuingCertificateRetriever(certificateRetriever
                ).SetOnlineFetching(RevocationDataValidator.OnlineFetching.NEVER_FETCH).AddOcspClient(ocspClient1).AddOcspClient
                (ocspClient2);
            validator.SetFreshness(-2 * MILLISECONDS_PER_DAY);
            validator.Validate(report, checkCert, checkDate);
            NUnit.Framework.Assert.AreEqual(0, report.GetFailures().Count);
        ...
        public virtual void NoRevocationDataTest() {
            ValidationReport report = new ValidationReport();
            RevocationDataValidator validator = new RevocationDataValidator().SetOnlineFetching(RevocationDataValidator.OnlineFetching
                .NEVER_FETCH);
        ...
        public virtual void SortResponsesTest() {
            ...
            certificateRetriever.AddTrustedCertificates(JavaCollectionsUtil.SingletonList(caCert));
            RevocationDataValidator validator = new RevocationDataValidator().SetIssuingCertificateRetriever(certificateRetriever
                ).SetOnlineFetching(RevocationDataValidator.OnlineFetching.NEVER_FETCH).SetCRLValidator(new CRLValidator
                ().SetFreshness(-5 * MILLISECONDS_PER_DAY)).AddCrlClient(crlClient).AddOcspClient(ocspClient1).AddOcspClient
                (ocspClient2).AddOcspClient(ocspClient3);
            validator.Validate(report, checkCert, checkDate);
            NUnit.Framework.Assert.AreEqual(0, report.GetFailures().Count);
            ...
            NUnit.Framework.Assert.AreEqual(MessageFormatUtil.Format(CRLValidator.FRESHNESS_CHECK, checkDate.AddDays(2
                ), checkDate, -5 * MILLISECONDS_PER_DAY), item.GetMessage());
            ...
            NUnit.Framework.Assert.AreEqual(MessageFormatUtil.Format(CRLValidator.FRESHNESS_CHECK, checkDate, checkDate
                , -5 * MILLISECONDS_PER_DAY), item.GetMessage());
